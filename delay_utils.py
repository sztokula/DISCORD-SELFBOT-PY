...
class DelayController:
    ...
    def _get_state(self, account_id):
        ...
        state = {
            "dm_since_pause": 0,
            "dm_since_idle": 0,
            "pause_every": random.randint(2, 5),
            "idle_every": random.randint(4, 9),
            "last_send_ts": 0.0,
        }
    ...
    def _diurnal_pause(self):
        hour = time.localtime().tm_hour
        if 0 <= hour < 6:
            return random.uniform(120.0, 600.0)
        if 6 <= hour < 9:
    ...
    def next_delay(
        ...
        *,
        ...
        base = gaussian_delay(base_min, base_max)
        if base <= 0:
        ...
        with self._lock:
            ...
            if recently_reconnected:
                ...
                state["pause_every"] = random.randint(2, 5)
                state["idle_every"] = random.randint(4, 9)
            ...
            if state["dm_since_pause"] >= state["pause_every"] and did_send:
                extra += random.uniform(5.0, 25.0)
                state["dm_since_pause"] = 0
                state["pause_every"] = random.randint(2, 5)

            if state["dm_since_idle"] >= state["idle_every"] and did_send:
                extra += random.uniform(120.0, 900.0)
                state["dm_since_idle"] = 0
                state["idle_every"] = random.randint(4, 9)

            extra += self._diurnal_pause()
        ...
        jitter = random.uniform(0.0, max(1.0, delay * 0.1))
